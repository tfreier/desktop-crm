...
public class CrmManager
{
	...
	String query = "calls.assigned_user_id='" + userId + "' "
		+ "and calls.status='Planned' and calls.date_start between " + "'" + formatter.print(from) + "' and '"
		+ formatter.print(to) + "'";
	...
	String query = "meetings.assigned_user_id='" + userId + "' "
		+ "and meetings.status='Planned' and meetings.date_start between " + "'" + formatter.print(from)
		+ "' and '" + formatter.print(to) + "'";
    ...
    private static <T extends AbstractCrmObject> T loadCrmObjectByNumber(final CrmObjectCreator<T> creator,
	    String moduleName, String number)
    {
	checkSetup();
    ...
    private static <T extends Lead> Collection<T> loadCrmContactBySearch(final CrmObjectCreator<T> creator,
	    String moduleName, String search)
    {
	...
	try
	{
	    List<SugarEntity> result = api.getRelationsships(session, "Accounts", accountId, "opportunities",
    ...
    private static <T extends AbstractCrmObject> Collection<T> loadCrmObjects(final CrmObjectCreator<T> creator,
	    String moduleName, String query)
    {
	return loadCrmObjects(creator, moduleName, query, null);
    }

    private static <T extends AbstractCrmObject> Collection<T> loadCrmObjects(final CrmObjectCreator<T> creator,
	    String moduleName, String query, String orderBy)
    {
     ...
     * @return
     */
    private static <T extends AbstractCrmObject> Collection<T> convertEntityList(final CrmObjectCreator<T> creator,
	    String moduleName, final List<SugarEntity> beans)
    {
     ...
     * @return
     */
    private static <T extends AbstractCrmObject> T convertEntity(final CrmObjectCreator<T> creator, String moduleName,
	    SugarEntity entity)
    {
	...
	try
	{
	    ...
	    String oldDueStr = formatter.parseDateTime(bean.get("date_due")).plusHours(gmtOffset)
		    .toDateTime(DateTimeZone.getDefault()).toString("yyyy-MM-dd hh:mm");
	...
	try
	{
	    ...
	    String oldDueStr = formatter.parseDateTime(bean.get("date_start")).plusHours(gmtOffset)
		    .toDateTime(DateTimeZone.getDefault()).toString("yyyy-MM-dd hh:mm");
	    sb.append("Rescheduled from ").append(oldDueStr).append(" to ").append(newDueStr);
     ...
     * @param creator
     */
    private static <T extends AbstractCrmObject> T loadCrmObject(String id, final String moduleName,
	    CrmObjectCreator<T> creator)
    {
	checkSetup();
	try
	{
